...
class BottomProductSheetFragment(
    ...
    private fun especialBinding() {
        if (cart != null) {
            ...
            binding.etQuantity.text =
                Editable.Factory.getInstance().newEditable(cart.quantity.toString())
        }
    }
    ...
    private fun setupListeners() {
        ...
        binding.btnLess.setOnClickListener {
            ...
            binding.etQuantity.text =
                Editable.Factory.getInstance().newEditable(newQuantity.toString())
        }
    ...
    private fun addToMyList() {
        GlobalScope.launch {
            if (product.isInMyList()) {
                (context?.applicationContext as App).getMyListAS().delete(product){
                    setupViewMyList(false)
                    product.setInMyList(false)
                }
            } else {
                (context?.applicationContext as App).getMyListAS().insert(product) {
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        if (cart != null) {
            (context?.applicationContext as App).getCartAS()
                .changeQuantity(cart, binding.etQuantity.text.toString().toInt())
        }
        observer?.updateView()
        _binding = null
    }
